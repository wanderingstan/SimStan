	...
	$.widget("ui.weekCalendar", {
	    ...
	    _setupEventDelegation : function() {
	        ...
	        this.element.click(function(event) {
	            ...
	            } else if($target.parent().parent().hasClass("cal-event")) { // SimStan
	                options.eventClick($target.parent().parent().data("calEvent"), $target.parent().parent(), event); // SimStan
	            } else if($target.parent().parent().parent().hasClass("cal-event")) { // SimStan
	                options.eventClick($target.parent().parent().parent().data("calEvent"), $target.parent().parent().parent(), event); // SimStan
	            } else if($target.parent().parent().parent().parent().hasClass("cal-event")) { // SimStan
	                options.eventClick($target.parent().parent().parent().parent().data("calEvent"), $target.parent().parent().parent().parent(), event); // SimStan
	            }
	    ...
	    _renderCalendar : function() {
	        ...
	        if(options.buttons) {
	            calendarNavHtml = "<div class=\"calendar-nav\">\
	                <span id=\"calendar-nav-left\"></span>\
	                <button class=\"today ui-state-default ui-corner-all\">" + options.buttonText.today + "</button>\
	                <button class=\"prev ui-state-default ui-corner-all ui-state-hover\">" + options.buttonText.lastWeek + "</button>\
	                <button class=\"next ui-state-default ui-corner-all ui-state-hover\">" + options.buttonText.nextWeek + "</button>\
	                <span id=\"calendar-nav-right\"></span>\
	                </div>"; // SimStan -- added jquery ui css classes, span's on either side for additional modifications
	    ...
	    _renderEvent: function (calEvent, $weekDay) {
	        ...
	        //$calEvent.css({lineHeight: (options.timeslotHeight - 2) + "px", fontSize: (options.timeslotHeight / 2) + "px"}); // SimStan 
	    ...
	    _groupOverlappingEventElements : function($weekDay) {
	        ...
	        $.each(sortedEvents, function(){
	            $(this).css({width: "96%", left: "", right: ""}); // SimStan: changed from 100% to allow room 
	    ...
	    _refreshEventDetails : function(calEvent, $calEvent) {
	        ...
	        var picUrl = (calEvent.picture ? calEvent.picture : eventweekOptions.userPicUrl);
	        $calEvent.find(".title").html('<div class="cal-user-picture-div"><img class="cal-user-picture" src="/' + picUrl + '"></div><b>' + calEvent.title + '</b> - ' + calEvent.body); // SimStan
        ...
        */
	    ...
	    _cleanDate : function(d) {
	        if (typeof d == 'string') {
	            return $.weekCalendar.parseISO8601(d, false) || Date.parse(d) || new Date(parseInt(d)); // SimStan: changed 2nd param of parseISO8601 to false, so timezones won't be ignored.
            }
	        if (typeof d == 'number') {
	            return new Date(d);
            }
...
//                if (d[12]) { date.setMilliseconds(Number("0." + d[12]) * 1000); } // simstan: d[12] is not milliseconds, but timezone offset
                if (!ignoreTimezone) {
//                    if (d[14]) {
...
//                    offset -= date.getTimezoneOffset();
                  if (d[11]) { // simstan
                    offsetMinutes = Number(d[11]) * 60; // simstan
                  } // simstan
                }
                return new Date(Number(date) + (offsetMinutes * 60 * 1000));
            }
        };
    }();
